  ...
  <div class="app-container">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 500px; margin-left:50px;">
        <el-form-item label="部门名" prop="stu_id">
...
<script>
import { getClassesList, createClasses, updateClasses, deleteClasses, filter } from '@/api/classes'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        class_name: [{ required: true, message: '请输入部门名', trigger: 'change' }],
      },
  ...
  methods: {
    getClassesList() {
      getClassesList(this.listQuery).then(response => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateClasses(this.temp.id, this.temp).then(() => {
