...
<script>
import { getCourseList, createCourse, updateCourse, deleteCourse, filter } from '@/api/course'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        course_name: [{ required: true, message: '请输入课程名', trigger: 'change' }],
      },
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateCourse(this.temp.id, this.temp).then(() => {
